...
def reraise(err: BaseException) -> None:
	...
	if isinstance(err, _uniffi.WebTransportError.SessionClosedByPeer):
		...
		new.source = closed_by  # type: ignore[assignment]
		new.code = code
		new.reason = reason
		raise new from err
	...
	if isinstance(err, _uniffi.WebTransportError.StreamClosedByPeer):
		...
		new.kind = err.kind  # type: ignore[assignment]
		new.code = err.code
		raise new from err
	...
	if isinstance(err, _uniffi.WebTransportError.StreamIncompleteRead):
		new = StreamIncompleteReadError(f"expected {err.expected} bytes, got {err.got} before EOF")
