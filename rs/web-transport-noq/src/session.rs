...
impl web_transport_trait::Session for Session {
    ...
    fn poll_open_uni(
        ...
        ops.open_uni.poll(cx, || {
            Self::open_uni_owned(conn.clone(), *header_uni, error.clone())
        })
    }
    ...
    fn poll_open_bi(
        ...
        ops.open_bi.poll(cx, || {
            Self::open_bi_owned(conn.clone(), *header_bi, error.clone())
        })
    }
    ...
    fn poll_closed(&mut self, cx: &mut Context<'_>) -> Poll<Self::Error> {
        ...
        ops.closed
            .poll(cx, || Self::closed_owned(conn.clone(), error.clone()))
    }
    ...
    fn poll_recv_datagram(&mut self, cx: &mut Context<'_>) -> Poll<Result<Bytes, Self::Error>> {
        ...
        ops.recv_datagram.poll(cx, || {
            Self::read_datagram_owned(conn.clone(), error.clone(), session_id)
        })
    }
