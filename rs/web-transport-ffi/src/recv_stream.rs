...
use crate::error::{WebTransportError, map_read_error, map_read_exact_error, map_read_to_end_error, map_session_error};
...
impl RecvStream {
	...
	where
		R: Send + 'static,
		F: FnOnce(tokio::sync::OwnedMutexGuard<web_transport_quinn::RecvStream>) -> Fut + Send + 'static,
...
impl RecvStream {
	...
	pub async fn read(&self, n: u64) -> Result<Vec<u8>, WebTransportError> {
		...
		self.cancellable(move |mut guard| async move {
			...
			let chunk = guard.read_chunk(n as usize, true).await.map_err(map_read_error)?;
			match chunk {
	...
	pub async fn wait_closed(&self) -> Result<Option<u32>, WebTransportError> {
		self.cancellable(|mut guard| async move { guard.received_reset().await.map_err(map_session_error) })
			.await
	}
}
