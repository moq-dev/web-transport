...
use crate::error::{WebTransportError, map_session_error};
...
impl Session {
	...
	pub async fn open_bi(&self) -> Result<BiStream, WebTransportError> {
		...
		let handle = RUNTIME.spawn(async move { session.open_bi().await.map_err(map_session_error) });
	...
	pub async fn open_uni(&self) -> Result<Arc<SendStream>, WebTransportError> {
		let session = self.session().await?;
		let handle = RUNTIME.spawn(async move { session.open_uni().await.map_err(map_session_error) });
		let send = handle
			.await
	...
	pub async fn accept_bi(&self) -> Result<BiStream, WebTransportError> {
		let session = self.session().await?;
		let handle = RUNTIME.spawn(async move { session.accept_bi().await.map_err(map_session_error) });
		let (send, recv) = handle
			.await
	...
	pub async fn accept_uni(&self) -> Result<Arc<RecvStream>, WebTransportError> {
		let session = self.session().await?;
		let handle = RUNTIME.spawn(async move { session.accept_uni().await.map_err(map_session_error) });
		let recv = handle
			.await
	...
	pub async fn receive_datagram(&self) -> Result<Vec<u8>, WebTransportError> {
		let session = self.session().await?;
		let handle = RUNTIME.spawn(async move { session.read_datagram().await.map_err(map_session_error) });
		let data = handle
			.await
