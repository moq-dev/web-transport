...
use crate::error::{WebTransportError, map_client_error};
...
impl Client {
	...
	pub fn new(config: ClientConfig) -> Result<Arc<Self>, WebTransportError> {
		...
		let mut tls_config = if config.no_cert_verification {
			...
			tls_builder.with_root_certificates(roots).with_no_client_auth()
		};
	...
	pub async fn connect(&self, url: String) -> Result<Arc<Session>, WebTransportError> {
		...
		let handle = RUNTIME.spawn(async move { client.connect(parsed).await.map_err(map_client_error) });
	...
	pub fn close(&self, code: u64, reason: String) -> Result<(), WebTransportError> {
		let var_code =
			quinn::VarInt::from_u64(code).map_err(|_| WebTransportError::invalid("code must be < 2^62"))?;
		let _guard = RUNTIME.enter();
