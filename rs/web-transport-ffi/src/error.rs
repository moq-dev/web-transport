//! Flat error enum exported to UniFFI.
//!
...
pub fn map_connection_error(err: quinn::ConnectionError) -> WebTransportError {
	match err {
		...
		quinn::ConnectionError::ApplicationClosed(ref close) => {
			let code = web_transport_quinn::proto::error_from_http3(close.error_code.into_inner()).map(|c| c as u64);
		...
		quinn::ConnectionError::TransportError(ref te) => WebTransportError::protocol(te.to_string()),
		quinn::ConnectionError::VersionMismatch => WebTransportError::protocol("QUIC version mismatch"),
		quinn::ConnectionError::CidsExhausted => WebTransportError::protocol("connection IDs exhausted"),
	}
}
...
pub fn map_session_error(err: web_transport_quinn::SessionError) -> WebTransportError {
	match err {
		...
		web_transport_quinn::SessionError::WebTransportError(ref wte) => match wte {
			web_transport_quinn::WebTransportError::Closed(code, reason) => WebTransportError::SessionClosedByPeer {
...
pub fn map_read_to_end_error(err: web_transport_quinn::ReadToEndError, limit: usize) -> WebTransportError {
	match err {
		web_transport_quinn::ReadToEndError::TooLong => WebTransportError::StreamTooLong { limit: limit as u64 },
		web_transport_quinn::ReadToEndError::ReadError(re) => map_read_error(re),
	}
}

pub fn map_read_exact_error(err: web_transport_quinn::ReadExactError, expected: usize, buf: &[u8]) -> WebTransportError {
	match err {
		web_transport_quinn::ReadExactError::FinishedEarly(bytes_read) => WebTransportError::StreamIncompleteRead {
...
pub fn map_server_error(err: web_transport_quinn::ServerError) -> WebTransportError {
	match err {
		...
		| web_transport_quinn::ServerError::Rustls(_) => WebTransportError::protocol(err.to_string()),
	}
}
