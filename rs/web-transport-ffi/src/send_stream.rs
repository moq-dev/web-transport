...
use crate::error::{WebTransportError, map_session_error, map_write_error};
...
impl SendStream {
	...
	where
		R: Send + 'static,
		F: FnOnce(tokio::sync::OwnedMutexGuard<web_transport_quinn::SendStream>) -> Fut + Send + 'static,
...
impl SendStream {
	...
	pub async fn write(&self, data: Vec<u8>) -> Result<(), WebTransportError> {
		self.cancellable(|mut guard| async move { guard.write_all(&data).await.map_err(map_write_error) })
			.await
	}
	...
	pub async fn write_some(&self, data: Vec<u8>) -> Result<u64, WebTransportError> {
		self.cancellable(|mut guard| async move {
			guard.write(&data).await.map(|n| n as u64).map_err(map_write_error)
		})
		.await
	}
	...
	pub async fn finish(&self) -> Result<(), WebTransportError> {
		self.cancellable(|mut guard| async move {
			guard.finish().map_err(|_| WebTransportError::StreamClosedLocally)
		})
		.await
	}
